...
public:
    ...
    virtual float forward(const uint8_t *y_true, const float *y_pred, int batch_size, int num_classes, dim3 blockSize) = 0;
    virtual void backward(const uint8_t *y_true, const float *y_pred, int batch_size, int num_classes, float *gradients, dim3 blockSize) = 0;
