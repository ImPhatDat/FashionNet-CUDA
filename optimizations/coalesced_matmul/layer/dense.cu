...
__global__ void matmul_kernel_version1(const float *A, const float *B, float *C, int M, int K, int N, int blockdim) {
    ...
    const int row = blockIdx.y * blockdim + (threadIdx.x % blockdim);
...
void Dense::matmul(const float *A, const float *B, float *C, int M, int K, int N, dim3 blockSize) {
    ...
    if (this->version == 0)
        ...
        dim3 block2(blockSize.x * blockSize.y);
        matmul_kernel_version1<<<gridSize, block2>>>(A, B, C, M, K, N, blockSize.x);
    }
