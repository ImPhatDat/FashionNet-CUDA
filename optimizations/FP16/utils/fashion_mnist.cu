...
void FashionMnist::prepareBatchesWithLabels(int batch_size, int input_size, __half **batches, uint8_t **batch_labels)
{
    ...
    for (size_t batch = 0; batch < num_batches; ++batch)
    {
        ...
        for (int i = 0; i < batch_size; ++i)
        {
            ...
            {
                batches[batch][i * input_size + j] = __float2half(static_cast<float>(image.pixels[j]) / 255.0f);
            }
